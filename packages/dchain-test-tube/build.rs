...
fn main() {
    ...
    let prebuilt_lib_dir = manifest_dir.join("libdchaintesttube").join("artifacts");

    let lib_name = "dchaintesttube";

    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());

    let header = if std::env::var("DOCS_RS").is_ok() {
        manifest_dir
            .join("libdchaintesttube")
            .join("artifacts")
            .join("libdchaintesttube.docrs.h")
    } else {
        out_dir.join(format!("lib{}.h", lib_name))
    };
    // rerun when go code is updated
    println!("cargo:rerun-if-changed=./libdchaintesttube");
    ...
    if env::var("PREBUILD_LIB") == Ok("1".to_string()) {
        build_libdchaintesttube(prebuilt_lib_dir.join(lib_filename));
    }
    ...
    if std::fs::metadata(&out_dir_lib_path).is_err()
        || env::var("DCHAIN_TEST_TUBE_DEV") == Ok("1".to_string())
    {
        build_libdchaintesttube(out_dir_lib_path);
    }
    ...
    if env::var("PROFILE").unwrap() == "debug" {
        ...
        // for each file with pattern `libdchaintesttube.*`, copy to target dir
        for entry in std::fs::read_dir(out_dir.clone()).unwrap() {
            let entry = entry.unwrap();
            let path = entry.path();
            if path.is_file() {
                let file_name = path.file_name().unwrap().to_str().unwrap();
                if file_name.starts_with("libdchaintesttube") {
...
fn build_libdchaintesttube(out: PathBuf) {
    ...
    let tidy_status = Command::new("go")
        .current_dir(manifest_dir.join("libdchaintesttube"))
        .arg("mod")
        .arg("tidy")
        .spawn()
        .unwrap()
