...
import (
	...
	// dchain-test-tube
	"github.com/d-foundation/test-tube/dchain-test-tube/result"
	"github.com/d-foundation/test-tube/dchain-test-tube/testenv"
)
...
func initTestEnvInternal(platformAdmin string) uint64 {
	...
	nodeHome, err := os.MkdirTemp("", ".dchain-test-tube-temp-")
	if err != nil {
		panic(err)
	}

	env := new(testenv.TestEnv)
	env.App = testenv.NewDChainApp(nodeHome)
	...
	// NOTE: No SetupParamTypes for DChain
